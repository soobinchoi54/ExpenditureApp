...
function Expenditure() {
  ...
  const addTransaction = () => {
    Axios.post("http://localhost:3001/create", {
      ...
      amount: getAmountInOrOut(amount, category),
    }).then(() => {
      setTransaction([
        ...transactionList,
        {
  ...
  const getAmountInOrOut = (amount, category) => {
    var realAmount = category === "income" ? amount : -amount;
    return realAmount;
  };
  ...
  return (
    <div className="AddExpenditure">
      ...
      <div className="GIF-container"><img src={gif}></img></div>
    </div>
  );
}
